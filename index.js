...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        try {
            if (m.type !== 'notify') return; // Only process new live incoming messages
            ...
            // --- 1. TRIGGER DETECTION (WELCOME MESSAGE) ---
...
📌 _Simply reply to this message with your details, and our team will contact you shortly!_`;
            ...
            // --- 2. HANDLE WAITING CONFIRMATION ---
            if (userStates[sender] && userStates[sender].state === 'WAITING_CONFIRMATION') {
                const confirmWords = ['yes', 'y', 'haan', 'correct', 'yep', 'ok', 'okay', 'perfect', 'done', 'ha', 'han'];
...
🌟 _Trust. Transparency. Excellence._ 🌟`;
                    ...
                    // Send Image Banner at the end as requested
                    try {
                        ...
                        console.log("[BOT] Image send skipped:", imgErr.message);
                    }
...
━━━━━━━━━━━━━━━━━━`;
            ...
            // --- 3. DETAILS DETECTION & CONFIRMATION ---
            ...
            if (hasDetails && (!userStates[sender] || userStates[sender].state !== 'WAITING_CONFIRMATION')) {
                ...
                // If we still didn't parse anything meaningful, fallback
